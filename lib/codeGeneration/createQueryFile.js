...
async function createQueryFile(currentStack, source) {
    ...
    let queryFragmentsImport = `\n\nimport { ${inflections_1.allCaps(rootName)}_FRAGMENT, ${inflections_1.allCaps(rootName)}_CHILD_FRAGMENT } from './fragments';`;
    if (rootChildren.length === 0) {
        queryFragmentsImport = `\n\nimport { ${inflections_1.allCaps(rootName)}_FRAGMENT } from './fragments';`;
    }
    content += queryFragmentsImport;
    let queryFragmentsList = `\n\${${inflections_1.allCaps(rootName)}_FRAGMENT}\n\${${inflections_1.allCaps(rootName)}_CHILD_FRAGMENT}`;
    if (rootChildren.length === 0) {
        queryFragmentsList = `\n\${${inflections_1.allCaps(rootName)}_FRAGMENT}`;
    }
