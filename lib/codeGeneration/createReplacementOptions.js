    ...
    if (boilerPlate !== constants_1.boilerPlates[constants_1.associationTypes.MULTIPLE]) {
        ...
        Object.keys(children).map(child => {
            ...
            if (children[child] === constants_1.associationTypes.MULTIPLE) {
                ...
                childrenConstantDeclarations += `\n  const ${child} = ${type}.children.find(child => child.typeId === TYPE_${inflections_1.allCaps(child)}_ID);`;
                ...
                childrenTypeList += `, TYPE_${inflections_1.allCaps(child)}_ID`;
                childrenConstantDeclarations += `\n  const ${child} = ${type}.children.find(child => child.typeId === TYPE_${inflections_1.allCaps(child)}_ID).instances[0];`;
...
import ${childComponent} from '../${childComponent}'; `;
            childrenTypeList += '';
            childrenBody += `, TYPE_${inflections_1.allCaps(child)}_ID
< ${childComponent}
              ${child} = { ${child} }
              ${type}Id = {${type}.id}
