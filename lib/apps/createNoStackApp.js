...
const errorMessage = (details) => `installation error: ${details}. Check out the error log noStackLog.txt.  If needed, please contact NoStack support.`;
async function createNoStackApp(appName) {
    // shell.echo('-ne', '                          (0% installed.  installing react...)\r);
    // cli.action.start('starting installation', {stdout: true})
    logging_1.logProgress('installing create-react-app... this could take a few minutes...');
    ...
    shell.cd(appName);
    installationList.map((packageToInstall) => {
        logging_1.logProgress(`Installing ${packageToInstall}...`);
    ...
    //   shell.exit(1)
    // }
    logging_1.logProgress("Apollo and no-stack packages have been added... installing'...");
  ...
  should create: /home/yisrael/projects/temp/app275/node_modules/no-stack/dist/no-stack.js
    ...
    logging_1.logProgress(`Installation is complete.  Enter 'cd ${appName}' and 'npm start' to try running the newly installed app locally`);
}
