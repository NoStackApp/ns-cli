...
async function createNoStackApp(appName, baseApp) {
    if (baseApp) {
            ...
            },
            {
                ...
                task: async () => {
                    ...
                    if (appDirectoryAlreadyExists) {
                        throw new Error(errorMessage_1.errorMessage(`the folder for ${appName} already exists. Please choose another appName.`));
                    }
                    await execa('cp', ['-r', baseApp, appName]).catch((error) => {
    ...
    const tasksFullInstallation = new Listr([
        {
            ...
            task: async () => {
                ...
                if (isAppFolder) {
                    throw new Error(errorMessage_1.errorMessage(`a folder for ${appName} already exists. Please choose a different app name`));
                }
                const upperCaseCheck = /(.*[A-Z].*)/;
                if (upperCaseCheck.test(appName)) {
                    throw new Error(errorMessage_1.errorMessage(`The ${appName} contains at least one capital, which create-react-app does not permit.`));
                }
                await execa('npx', ['create-react-app', appName, `>> ${LOGFILE}`]).catch((error) => {
                    throw new Error(`${chalk_1.default.red('error running create-react-app.')} You may try calling 'create-react-app ${appName}' directly and see what messages are reported. Here is the error reported:\n${error}`);
                });
            }
        },
        {
            ...
            task: async () => {
                return new Listr(installationList.map((item) => {
                    return {
                        title: item,
                        task: async () => {
                            await execa('npm', ['install', '--prefix', appName, '--save', item]).catch((error) => {
