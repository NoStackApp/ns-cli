...
const newUserClass = (args) => {
    ...
    // @ts-ignore
    stack.types[name] = {
        const: `TYPE_${inflections_1.allCaps(name)}_ID`,
        name,
...
const newSource = (args) => {
    const name = args[0];
    // @ts-ignore
    stack.sources[name] = {
        const: `SOURCE_${inflections_1.allCaps(name)}_ID`,
        name,
        tree: {},
        selections: [],
        constraints: {},
...
const newType = (args) => {
    ...
    if (assnType === 'multiple') {
        // @ts-ignore
        stack.types[name].sources[source].plural = inflection.camelize(pluralize(name), false);
    }
    // @ts-ignore
    ...
    stack.actions[`${source}__${name}_create`] = {
        const: `CREATE_${inflections_1.allCaps(name)}_FOR_${inflections_1.allCaps(source)}_ACTION_ID`,
        type: name,
        source,
        parentType,
    };
    ...
    stack.actions[`${source}__${name}_delete`] = {
        const: `DELETE_${inflections_1.allCaps(name)}_FOR_${inflections_1.allCaps(source)}_ACTION_ID`,
        type: name,
        source,
    };
};
const newSelection = (args) => {
...
async function generateJSON(template, appDir) {
    ...
    const streamResult = new Promise((resolve, reject) => {
        stream.on('end', async function () {
            await fs.outputJson(`${appDir}/stack.json`, stack, (err) => {
                if (err) {
                    // @ts-ignore
    ...
    const stackReturned = await streamResult;
