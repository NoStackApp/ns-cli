...
class Quickstarter extends command_1.Command {
    async run() {
        ...
        const appName = flags.appName || isRequired_1.isRequired('appName', 'quickstarter', '-a');
        const baseApp = flags.baseApp || '';
        const stack = flags.stack || isRequired_1.isRequired('stack', 'quickstarter', '-s');
        const flowSpec = flags.template || isRequired_1.isRequired('flowSpec', 'quickstarter', '-t');
        const user = flags.user || isRequired_1.isRequired('user', 'quickstarter', '-u');
        const userClass = flags.userClass || isRequired_1.isRequired('userClass', 'quickstarter', '-c');
        const password = flags.password || isRequired_1.isRequired('password', 'quickstarter', '-w');
        const email = flags.email || isRequired_1.isRequired('email', 'quickstarter', '-e');
        const licenseId = flags.licenseId || isRequired_1.isRequired('licenseId', 'quickstarter', '-l');
        let userInfo = {
            name: user,
            stack,
            password,
