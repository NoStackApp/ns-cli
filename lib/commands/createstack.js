...
class Createstack extends command_1.Command {
    async run() {
        ...
        const stack = flags.stack || isRequired_1.isRequired('stack', 'createstack', '-s');
        const user = flags.user || isRequired_1.isRequired('user', 'createstack', '-u');
        const password = flags.password || isRequired_1.isRequired('password', 'createstack', '-w');
        const email = flags.email || isRequired_1.isRequired('email', 'createstack', '-e');
        const licenseId = flags.licenseId || isRequired_1.isRequired('licenseId', 'createstack', '-l');
        let userInfo = {
            name: user,
            stack,
            password,
